...
\usepackage{tikz}
\usetikzlibrary{chains, positioning, arrows.meta, shapes.geometric}
...
\begin{tikzpicture}[node distance=1.6cm]
    	\tikzset{end/.style={rectangle,draw=blue!60,fill=blue!10,rounded corners,minimum width=2.2cm,minimum height=0.8cm,align=center},
...
\begin{figure}[!htbp]
    ...
    \begin{tikzpicture}[x=1.0cm,y=0.6cm]
        	\tikzset{bitgrid/.style={gray!50,thin}}
        % 全局参数
        \def\bits{8}
    \def\labels{{1,0,1,1,0,0,1,0}}
        % 画位栅格与位标注
        \foreach \i in {0,...,8} {
            \draw[bitgrid] (\i,4.6) -- (\i,-0.6);
        }
        \foreach \y/\name in {4.0/NRZ-L,3.0/NRZ-I,2.0/曼彻斯特,1.0/差分曼彻斯特,0.0/AMI} {
            \draw[bitgrid] (0,\y) -- (8,\y);
            \node[left] at (0,\y) {\scriptsize \name};
        }
        \foreach \i [count=\c from 1] in \labels {
            \node[below] at (\c-0.5,-0.4) {\scriptsize \i};
        }
        ...
        \begin{scope}
            ...
            % 手工根据 10110010 描绘
            \draw[thick]
                (0,\y+0.6) -- (1,\y+0.6) -- (1,\y-0.6) -- (2,\y-0.6) --
                (2,\y+0.6) -- (3,\y+0.6) -- (4,\y+0.6) -- (4,\y-0.6) --
                (6,\y-0.6) -- (6,\y+0.6) -- (7,\y+0.6) -- (7,\y-0.6) -- (8,\y-0.6);
        \end{scope}

        % ========== NRZ-I: 1=边界翻转, 0=不变 (初始低) ==========
        \begin{scope}
            \def\y{3.0}
            % 边界：1->翻转，0->保持；按 10110010 手工推导路径
            % 初始低(-0.6)
            \draw[thick]
                (0,\y-0.6) -- (0,\y+0.6) -- (1,\y+0.6)  % 第1位=1: 翻到高
                -- (2,\y+0.6)                             % 第2位=0: 保持高
                -- (2,\y-0.6) -- (3,\y-0.6)               % 第3位=1: 翻到低
                -- (3,\y+0.6) -- (4,\y+0.6)               % 第4位=1: 翻到高
                -- (5,\y+0.6)                              % 第5位=0: 保持高
                -- (5,\y-0.6) -- (6,\y-0.6)               % 第6位=0: 保持低（此处为示意，加入回落边以区分眼图）
                -- (6,\y+0.6) -- (7,\y+0.6)               % 第7位=1: 翻到高
                -- (7,\y-0.6) -- (8,\y-0.6);              % 第8位=0: 保持低
        \end{scope}

            % ========== Manchester: 半位翻转(1: 高->低, 0: 低->高) ==========
            \begin{scope}
                \def\y{2.0}
                % 位1: 1 => 高-低
                \draw[thick] (0,\y+0.6) -- (0.5,\y+0.6) -- (0.5,\y-0.6) -- (1,\y-0.6);
                % 位2: 0 => 低-高
                \draw[thick] (1,\y-0.6) -- (1.5,\y-0.6) -- (1.5,\y+0.6) -- (2,\y+0.6);
                % 位3: 1 => 高-低
                \draw[thick] (2,\y+0.6) -- (2.5,\y+0.6) -- (2.5,\y-0.6) -- (3,\y-0.6);
                % 位4: 1 => 高-低（起始需抬到高）
                \draw[thick] (3,\y-0.6) -- (3,\y+0.6) -- (3.5,\y+0.6) -- (3.5,\y-0.6) -- (4,\y-0.6);
                % 位5: 0 => 低-高
                \draw[thick] (4,\y-0.6) -- (4.5,\y-0.6) -- (4.5,\y+0.6) -- (5,\y+0.6);
                % 位6: 0 => 低-高（起始需降到低）
                \draw[thick] (5,\y+0.6) -- (5,\y-0.6) -- (5.5,\y-0.6) -- (5.5,\y+0.6) -- (6,\y+0.6);
                % 位7: 1 => 高-低
                \draw[thick] (6,\y+0.6) -- (6.5,\y+0.6) -- (6.5,\y-0.6) -- (7,\y-0.6);
                % 位8: 0 => 低-高
                \draw[thick] (7,\y-0.6) -- (7.5,\y-0.6) -- (7.5,\y+0.6) -- (8,\y+0.6);
            \end{scope}

            % ========== Differential Manchester: 中点必翻；0=起始翻，1=起始不翻 ==========
            \begin{scope}
                \def\y{1.0}
                % 位1: 1 => 起始不翻；前半低，半位翻到高
                \draw[thick] (0,\y-0.6) -- (0.5,\y-0.6) -- (0.5,\y+0.6) -- (1,\y+0.6);
                % 位2: 0 => 起始翻(高->低)，半位翻(低->高)
                \draw[thick] (1,\y+0.6) -- (1,\y-0.6) -- (1.5,\y-0.6) -- (1.5,\y+0.6) -- (2,\y+0.6);
                % 位3: 1 => 起始不翻；前半高，半位翻到低
                \draw[thick] (2,\y+0.6) -- (2.5,\y+0.6) -- (2.5,\y-0.6) -- (3,\y-0.6);
                % 位4: 1 => 起始不翻；前半低，半位翻到高
                \draw[thick] (3,\y-0.6) -- (3.5,\y-0.6) -- (3.5,\y+0.6) -- (4,\y+0.6);
                % 位5: 0 => 起始翻(高->低)，半位翻到高
                \draw[thick] (4,\y+0.6) -- (4,\y-0.6) -- (4.5,\y-0.6) -- (4.5,\y+0.6) -- (5,\y+0.6);
                % 位6: 0 => 起始翻(高->低)，半位翻到高
                \draw[thick] (5,\y+0.6) -- (5,\y-0.6) -- (5.5,\y-0.6) -- (5.5,\y+0.6) -- (6,\y+0.6);
                % 位7: 1 => 起始不翻；前半高，半位翻到低
                \draw[thick] (6,\y+0.6) -- (6.5,\y+0.6) -- (6.5,\y-0.6) -- (7,\y-0.6);
                % 位8: 0 => 起始翻(低->高)，半位翻到低
                \draw[thick] (7,\y-0.6) -- (7,\y+0.6) -- (7.5,\y+0.6) -- (7.5,\y-0.6) -- (8,\y-0.6);
            \end{scope}
            ...
            \begin{scope}
                \def\y{0.0}
                % 位1=1: +A
                \draw[thick] (0,\y) -- (0,\y+0.6) -- (1,\y+0.6) -- (1,\y);
                % 位2=0: 0电平
                \draw[thick] (1,\y) -- (2,\y);
                % 位3=1: -A
                \draw[thick] (2,\y) -- (2,\y-0.6) -- (3,\y-0.6) -- (3,\y);
                % 位4=1: +A
                \draw[thick] (3,\y) -- (3,\y+0.6) -- (4,\y+0.6) -- (4,\y);
                % 位5=0, 位6=0
                \draw[thick] (4,\y) -- (6,\y);
                % 位7=1: -A
                \draw[thick] (6,\y) -- (6,\y-0.6) -- (7,\y-0.6) -- (7,\y);
                % 位8=0
                \draw[thick] (7,\y) -- (8,\y);
            \end{scope}
    ...
    \end{tikzpicture}}
    \vspace{-3mm}
    \caption{典型线路编码波形示意（位序列：10110010；方向示意不作为考点）}
\end{figure}

% \noindent 常见编码要点与对比：

% {\small\setlength{\tabcolsep}{4pt}\renewcommand{\arraystretch}{1.15}
% \begin{longtable}{p{0.12\textwidth} p{0.28\textwidth} p{0.12\textwidth} p{0.12\textwidth} p{0.14\textwidth} p{0.22\textwidth}}
% \caption{常见数字线码对比}\label{tab:line-codes}\\
% \toprule
% \textbf{编码} & \textbf{基本思想} & \textbf{自同步} & \textbf{直流分量} & \textbf{带宽占用(相对)} & \textbf{典型问题/应用}\\
% \midrule
% \endfirsthead
% \toprule
% \textbf{编码} & \textbf{基本思想} & \textbf{自同步} & \textbf{直流分量} & \textbf{带宽占用(相对)} & \textbf{典型问题/应用}\\
% \midrule
% \endhead
% NRZ-L & 1高0低，位内不翻转 & 差 & 大（有DC） & 低 & 容易\textbf{基线漂移}且难恢复时钟；早期串口/磁记录 \\
% NRZ-I & 1引起边界翻转，0保持 & 一般 & 中等 & 低 & 连续0串时过渡少；FDDI等配合4B5B使用 \\
% RZ & 位中回到0电平（占空比50\%） & 较好 & 中等 & 中等 & 过渡多，频带较宽；部分光存储历史 \\
% 曼彻斯特 & 每位中点必翻，1/0用相反方向 & 好 & 小（近零） & 高（约为比特率的2倍主瓣） & \textbf{强自同步}；10BASE-T以太网 \\
% 差分曼彻斯特 & 中点必翻，0起始翻/1不翻 & 好 & 小（近零） & 高 & 对极性不敏感；令牌环等 \\
% AMI & 1在+A/-A间交替，0为0电平 & 一般 & 小（无DC） & 中等 & 长0串缺同步；配HDB3/B8ZS零替代提升同步 \\
% 4B5B(+NRZ-I/MLT-3) & 4位映射5位，限制最长0串(≤3) & 好 & 取决于后续线码 & 中等 & \textbf{25%编码开销}；FDDI/100BASE-TX（4B5B+MLT-3） \\
% 8B/10B & 8位映射10位，正负码保持DC平衡 & 好 & 极小（DC平衡） & 中等偏高 & \textbf{25%开销}；最长连同符号≤5；PCIe Gen1/2、FC、1000BASE-X \\
% \bottomrule
% \end{longtable}}
...
%     	\textbf{408考试提示}
%     \begin{itemize}
%         \item 识别关键词：\textbf{“中点必有翻转”→(差分)曼彻斯特；“1交替正负、0为零电平”→AMI；“1触发边界翻转”→NRZ-I}。
%         \item 以太网：10BASE-T用\textbf{曼彻斯特}；100BASE-TX常配\textbf{4B5B+MLT-3}；1000BASE-X/多高速串行常用\textbf{8B/10B}（新标准也见64B/66B）。
%         \item 选项陷阱：\textbf{自同步好}往往对应“中点必翻”的家族；\textbf{直流分量小/无}多见AMI、8B/10B；带宽利用率与同步性通常\textbf{难两全}。
%     \end{itemize}
% \end{minipage}}}%
% \end{center}
